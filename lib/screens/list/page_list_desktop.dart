...
class PagesListDesktop extends StatelessWidget {
  const PagesListDesktop({super.key});
  @override
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        if (firstVerseKey[0] == lastVerseKey[0]) {
          ...
          int realAyah =
              allChaptersInfo[int.parse(firstVerseKey[0]) - 1]['verses_count'];
          if (temAyah != realAyah) {
        ...
        if (surahNumberUnderPage > 0) {
          ...
          surahNumber = int.parse(lastVerseKey[0]);
          start = 0;
          ...
          if (surahNumberUnderPage == 2) {
            ...
            start = 0;
            end = allChaptersInfo[surahNumber - 1]['verses_count'];
        ...
        return GestureDetector(
          ...
          onTap: () async {
            ...
            Get.to(
              () => SuraView(
                surahNumber: int.parse(firstVerseKey[0]),
                start: int.parse(firstVerseKey[1]),
                end: int.parse(lastVerseKey[1]),
              ),
            );
          },
          child: Container(
            ...
            child: Column(
              children: [
                ...
                if (surahNumberUnderPage > 0) Column(children: surahUnderPage),
              ],
            ),
          ),
        );
